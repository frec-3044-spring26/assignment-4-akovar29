...
sunapee <- ice_data |> filter(lake_name == "Sunapee")
```
...
# Had to change the data type of ice_off_doy since it was a string and not a double

ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  geom_point() +
  geom_line() +
  labs(x = "Year", y = "Ice Off Day of Year", title = "Year vs. Ice Off Day of Year at Lake Sunapee")
```
...
Using the graph, it appears that there is a very slight, but noticeable trend downwards over time. Therefore, I expect the sign to be a minus for the slope of the relationship.
...
The direction of the slope is slightly negative, which lines up with the prediction I made from question 2.
...
**Answer 4:**
The R2 value for the regression model is 0.2055, which indicates that year explains very little of the variation in ice_off_doy and that they aren't strongly related.
...
```
Looking at the histogram, it does appear that the data has a mostly normal distribution as it's centered around zero and tapers off to the left and right with no noticeable outliers in frequency.
...
**Answer 7:**
According to the summary, the p-value for the slope is 2.504e-09.
...
**Answer 8:**
Using the slope value of -0.1060227, it would take about 7 years to create a 1 week change in ice off doy. This is meaningful as it shows the effects of climate change as warmer weather causes the ice off at Lake Sunapee to occur earlier every year.
...
**Answer 9:**
Since the p-value of the model is less than 0.05, it can be determined that the relationship between the year and the ice off day of year is statistically significant and therefore the model can be used for additional analysis/prediction.
...
```{r}
fit$coefficients[1] + fit$coefficients[2]*2040
#Calculated ice-off day in 2040: 102.6622

quantileSunapee <- quantile(sunapee$resid, c(0.05, 0.95))

102.6622 + quantileSunapee[1]
102.6622 + quantileSunapee[2]
```
The 90% confidence interval for my prediciton of the ice off day in 2040 is (83.2, 116.8), with the confidence interval having a width of about 33.6 days.
...
**Answer 12:**
```{r}
sunapeeBefore1970 <- filter(sunapee, year < 1970)
sunapeeAfter1970 <- filter(sunapee, year >= 1970)

fitBefore1970 <- lm(ice_off_doy ~ year, data = sunapeeBefore1970)
fitAfter1970 <- lm(ice_off_doy ~ year, data = sunapeeAfter1970)

ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  geom_point() +
  geom_line() +
  geom_abline(slope = fitBefore1970$coefficients[2], 
              intercept = fitBefore1970$coefficients[1], color = "blue") +
  geom_abline(slope = fitAfter1970$coefficients[2], 
              intercept = fitAfter1970$coefficients[1], color = "red") +
  labs(x = "Year", y = "Ice Off Day of Year", 
       title = "Year vs. Ice Off Day of Year at Lake Sunapee")
```

After using regression models to find a slope for the "sunapee" data before and after 1970 and adding them to the plot above, we can see that the sunapee data after 1970 (red line) clearly shows a much greater decrease in ice off day of year than the sunapee data before 1970 (blue line). With the data after 1970 having a considerably steeper slope, we can conclude that  the ice-off day of year is changing at a greater rate in recent years at Lake Sunapee.
